...
def read_regrid_weights(weight_path, n_dst):
    ...
    ESMF indices are 1-based and the caller subtracts 1. A file that was
    truncated -- by a killed job, a full disk, an interrupted transfer --
    returns the netCDF int fill (-2147483647) for `row`, which only shows up
    much later as "negative axis 0 index: -2147483648" out of scipy. Checking
    here costs nothing: these are the same arrays the caller needs anyway.

    Raises ValueError if the file cannot be trusted.
    """
    with xr.open_dataset(weight_path) as regrid_weight_ds:
        for name in ("col", "row", "S", "area_b"):
            if name not in regrid_weight_ds:
    ...
    # Zero weights is a valid answer, not damage: it means this granule's
    # footprint does not overlap the face. Treating it as corruption forces a
    # rebuild that produces the same empty file, and the second read then kills
    # the run. The index checks below are what catch a truncated file.
    if src_ind.size == 0:
        return src_ind, dst_ind, regrid_weights, dst_area

    if src_ind.min() < 0 or dst_ind.min() < 0:
        raise ValueError(
            f"index below zero: col min {src_ind.min()}, row min {dst_ind.min()}"
        )
    if dst_ind.max() >= n_dst:
        raise ValueError(f"row max {dst_ind.max()} outside the {n_dst}-cell grid")
...
def get_overlap_area_CSgrid(TROPOMI, filename, sat_ind, CSgridDir,
    ...
    try:
        ...
        # Rebuild once, then let a second failure propagate: at that point the
        # problem is the granule or the gridspec, not a damaged cache.
        print(
    ...
    # Accepted, but said out loud. The destination grid covers the whole globe,
    # so observations that map to nothing are unexpected rather than routine,
    # and a degenerate SCRIP grid looks exactly like this.
    if src_ind.size == 0:
        print(
            f"WARNING: {regrid_weight_fpath} holds no weights; this granule "
            f"maps to no grid cell and will contribute nothing.",
            flush=True,
        )
