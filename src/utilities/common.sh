...
# --- Robust replacement for `sbatch -W` --------------------------------
# Fix forever hang with `sbatch -W` on AWS pcluster
# `sbatch -W` (--wait) can hang when nested inside another Slurm job:
# the client sometimes misses the child job's completion and blocks
# indefinitely. This shadows `sbatch` so any call using -W/--wait instead
# submits normally and polls squeue until the job leaves the queue.
# Calls WITHOUT -W are passed through to the real sbatch, unchanged.
sbatch() {
    local want_wait=0 args=() a
    for a in "$@"; do
        if [[ "$a" == "-W" || "$a" == "--wait" ]]; then
            want_wait=1
            continue
    ...
    # No -W: behave exactly like the real sbatch.
    if [[ $want_wait -eq 0 ]]; then
        command sbatch "$@"
        return $?
    fi

    # -W requested: submit, then poll instead of blocking on the client.
    local jobid
    jobid=$(command sbatch --parsable "${args[@]}") || return $?
    jobid=${jobid%%;*}                       # strip ';cluster' if federated
    if [[ -z "$jobid" ]]; then
        echo "sbatch(wrapper): submission produced no job id" >&2
        return 1
    fi
    printf "Submitted batch job %s (polling for completion)\n" "$jobid"

    sleep 5                                   # let the controller register it
    while squeue -j "$jobid" -h -o %T 2>/dev/null | grep -q .; do
        sleep "${SBATCH_POLL_INTERVAL:-10}"
    done
    printf "Job %s finished (left queue) at %s\n" "$jobid" "$(date)"
    return 0
}